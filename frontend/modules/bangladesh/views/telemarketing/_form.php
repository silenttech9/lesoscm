...
if ($model->isNewRecord) {
    $product = ArrayHelper::map(LookupProgramProduct::find()->where(['module_id'=>$module_id])->asArray()->all(), 'id', 'program_product');
...
$telemarketers = ArrayHelper::map(User::find()->where(['module_id'=>$module_id])->asArray()->all(),'id','username'); 
    ...
    $product = ArrayHelper::map(LookupProgramProduct::find()->where(['module_id'=>$model->module_id])->asArray()->all(), 'id', 'program_product');


$telemarketers = ArrayHelper::map(User::find()->where(['module_id'=>$model->module_id])->asArray()->all(),'id','username'); 
