...
use frontend\modules\bangladesh\models\BangladeshStockSearch;
 ...
 */
class BangladeshStockController extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    public function actionUpload($country_id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $delete = $connection->createCommand('DELETE FROM upload_excel WHERE country_id = 8')->execute();


            $dir = $_SERVER['DOCUMENT_ROOT'].'/lscm/frontend/web/uploads/bangladesh/';
            ...
            if(count(scandir($dir)) == 2) {
                ...
                $model->filename->saveAs('uploads/bangladesh/' . $model->filename->baseName . '.' . $model->filename->extension);
                ...
                $model->excel_directory = $path.'uploads/bangladesh/';
                $model->save();

                $getFile = UploadExcel::find()->orderBy(['id' => SORT_DESC,'country_id'=>8])->limit(1)->one();
                ...
                $dirToDelete = $_SERVER['DOCUMENT_ROOT'].'/lscm/frontend/web/uploads/bangladesh/*';
        ...
        $connection = \Yii::$app->db;
        $delete = $connection->createCommand('DELETE FROM bangladesh_stock')->execute();
        $alter = $connection->createCommand('ALTER TABLE bangladesh_stock AUTO_INCREMENT = 1')->execute();


        $connection = \Yii::$app->db;
        $sql = $connection->createCommand('SELECT * FROM bangladesh_stock');
        $model = $sql->queryAll();
        ...
        if (empty($model)) {
           ...
           $connection = \Yii::$app->db;
            $sql = $connection->createCommand('DELETE FROM bangladesh_stock');
            $model = $sql->queryAll();
        }
    ...
    {
        $searchModel = new BangladeshStockSearch();
    ...
    {
        if (($model = BangladeshStock::findOne($id)) !== null) {
            return $model;
        } else {
