...
use frontend\modules\bangladesh\models\CustomerSearch;
...
class CustomerController extends Controller
{
    ...
    public function actionUpdate($id,$module_id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) ) {
            ...
            $model->save();
            Yii::$app->getSession()->setFlash('updatePic', 'Customer Information <b>'.$model->company_name.'</b> Successful Updated');
            return $this->redirect(['view', 'id' => $model->id]);
        } else {
    ...
    public function actionSearcht($id){
        ...
        $sql = $connection->createCommand('SELECT company_name,address FROM customer WHERE company_name LIKE "%'.$id.'%" AND module_id = 8');
        $model = $sql->queryAll();
