...
$country = ArrayHelper::map(LookupCountry::find()->where(['id'=>3])->asArray()->all(),'id','country'); 
...
$country_code = ArrayHelper::map(LookupPhoneCountryCode::find()->where(['active'=>['Yes'],'id'=>214])->asArray()->all(),'code','code');
...
<?php } else {?>
    ...
    <?php } ?>
       ...
       <?= Html::a('<i class="fa fa-users"></i>',FALSE, ['value'=>Url::to(['customer-pic/create','id'=>$model->id,'module_id'=>$model->module_id]),'class' => 'btn btn-circle btn-icon-only blue-chambray picCreate','id'=>'picCreate','title' => 'Add Customer PIC',]) ?>
    </p>
    <div class="caption">
                ...
                [
                    ...
                    'template'=>'{view}   {edit}   {delete}',
                        ...
                        'urlCreator' => function ($action, $model, $key, $index) {
                            ...
                            if ($action === 'edit') {
                                $url = ['value'=>Url::to(['customer-pic/update','id'=>$model->id,'module_id'=>3]),'class'=>'btn btn-circle btn-icon-only blue-chambray picUpdate','title'=>'Edit'];
                                return $url;
                            }
                            if ($action === 'delete') {
                                $url = ['customer-pic/delete','id'=>$model->id,'customer_id'=>$model->customer_id,'module_id'=>3];
                                return $url;
                            }
