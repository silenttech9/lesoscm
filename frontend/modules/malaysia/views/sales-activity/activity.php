...
$special = ArrayHelper::map(User::find()->joinWith(['role','akses'])->where(['user_role.user_role'=>['Specialist'],'user_state_akses.state_akses'=>$state_id])->asArray()->all(),'id','username'); 
...
<div class="sales-activity-log-form">
    ...
    <div class="show-pic-activity" style="display:none;">
            <div class="pic-activity"></div>

    </div>
    <span class='font-red-soft font-md'>Add More Customer PIC -
        <?= Html::a('Click Here',FALSE, ['value'=>Url::to(['createmorepic','cust_id'=>$mdl->customer_id,'id'=>Yii::$app->request->get('id')]),'class' => 'picCreate','id'=>'picCreate','title' => 'Add Customer PIC',]) ?>
    </span>
    <br>
    <br>
