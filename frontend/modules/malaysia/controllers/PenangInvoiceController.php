...
use frontend\modules\malaysia\models\PenangInvoiceSearch;
...
/**
 * PenangInvoiceController implements the CRUD actions for PenangInvoice model.
 */
class PenangInvoiceController extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    public function actionUpload($state_id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $delete = $connection->createCommand('DELETE FROM upload_invoice WHERE state_id = 23')->execute();


            $dir = $_SERVER['DOCUMENT_ROOT'].'/lscm/frontend/web/uploads/invoicepenang/';
            ...
            if(count(scandir($dir)) == 2) {
                ...
                $model->filename->saveAs('uploads/invoicepenang/' . $model->filename->baseName . '.' . $model->filename->extension);
                ...
                $model->excel_directory = $path.'uploads/invoicepenang/';
                $model->save();
                ...
                $dirToDelete = $_SERVER['DOCUMENT_ROOT'].'/lscm/frontend/web/uploads/invoicepenang/*';
        ...
        $connection = \Yii::$app->db;
        $delete = $connection->createCommand('DELETE FROM penang_invoice')->execute();
        $alter = $connection->createCommand('ALTER TABLE penang_invoice AUTO_INCREMENT = 1')->execute();


        $connection = \Yii::$app->db;
        $sql = $connection->createCommand('SELECT * FROM penang_invoice');
        $model = $sql->queryAll();
        ...
        if (empty($model)) {
           ...
           $connection = \Yii::$app->db;
            $sql = $connection->createCommand('DELETE FROM penang_invoice');
            $model = $sql->queryAll();
        } 
    ...
    /**
     * Lists all PenangInvoice models.
     * @return mixed
     */

    public function actionIndex($state_id)
    {
        $searchModel = new PenangInvoiceSearch();
    ...
    /**
     * Displays a single PenangInvoice model.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Creates a new PenangInvoice model.
    ...
    /**
     * Updates an existing PenangInvoice model.
    ...
    /**
     * Deletes an existing PenangInvoice model.
    ...
    /**
     * Finds the PenangInvoice model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return PenangInvoice the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = PenangInvoice::findOne($id)) !== null) {
            return $model;
        } else {
